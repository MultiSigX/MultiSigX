	...
	try
		{
			...
			var pubkeycompress = Crypto.util.bytesToHex(eckey.pub).toUpperCase();
			result = {'result':1, 'secret':hash_str, 'pubkey':pub, 'privkey':priv, 'payload':payload, 'payload_hash':Crypto.SHA256(payload), 'pubkeycompress':pubkeycompress,'response':'keys generated'};
		}
		catch(error)
		{
			alert(error);
