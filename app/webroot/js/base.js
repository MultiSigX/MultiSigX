...
var base = new function(){
	...
	{
		try{
			localStorage.setItem('btcmw.temp', JSON.stringify({test:'testing'}));
			return localStorage['btcmw.temp'];
		}catch(e){
			return false;
		}
	}
	...
	{
		localStorage.setItem('btcmw.'+key, JSON.stringify(value));
	}

	// GET FROM STORE
	this.get = function(key)
	{
		if(this.is_json(localStorage['btcmw.'+key]))
		{
			return $.parseJSON(localStorage['btcmw.'+key])
		}
		else if(localStorage['btcmw.'+key])
		{
			return localStorage['btcmw.'+key];
		}
		return false;
	}
